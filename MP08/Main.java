...
//        for (int i = 0; i < fileNames.length; i++) {
//            preprocessedCode[i] = builder.preprocess(code[i]);
//        }
...
//        exe.execute();
        ...
        builder.createProject(EXE_FILE_NAME);
        Executable exe2 = builder.build(code); //배열 버전
        exe2.execute();
        System.out.println();

        Builder builder2 = new IDE();
        builder2.createProject(EXE_FILE_NAME);
        System.out.println("\nIDE 리스트 사용");
        Executable exe3 = builder2.build(Arrays.stream(code).toList()); // 리스트 버전
        exe3.execute();
